...
export default function LoadingScreen({ isActive, targetName, onComplete }) {
  ...
  useEffect(() => {
    ...
    const steps = [
      ...
      { type: 'res', res: `Synthesizing runtime container... done (${Math.floor(Math.random() * 200 + 120)}KB optimized)`, cls: 'dim', delay: 2050 },
      { type: 'res', res: `✓ AI Sandbox isolated & unit test matrix verified (100% PASS)`, cls: 'ok', delay: 2320 },
      { type: 'cmd', prompt: '$', cmd: `deploy --target browser --env prod --ssl strict`, delay: 2550 },
