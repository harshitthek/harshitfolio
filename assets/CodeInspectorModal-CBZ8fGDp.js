...
class PhishShieldHeuristicExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        return np.array(features)`},pageshield:{filename:"background.js",language:"JavaScript",description:"Page Shield: Groq Llama 3.3 70B & Gemini 2.0 Flash background service worker with human-like persona prompting.",code:`/**
...
}`},ml:{filename:"autovaluate_stacking_model.py",language:"Python",description:"CatBoost + XGBoost Dual-Engine Stacking Regressor pipeline trained on 40,000+ vehicle transactions with native categorical embeddings and OOD bounds.",code:`import numpy as np
import pandas as pd
...
class AutoValuateDualEngine:
    ...
    def fit_and_evaluate(self, X_train, y_train, X_test, y_test):
        ...
        return {"r2": r2, "mae": mae}`},webhook:{filename:"webhook_receiver.py",language:"Python",description:"Constant-time cryptographic signature verification for GitHub App webhooks, preventing timing side-channel attacks.",code:`import hashlib
import hmac
import os
...
def handle_webhook():
    ...
    return {"status": "dispatched", "event": event_type}, 200`},yggdrasil:{filename:"yggdrasil_tree_agent.py",language:"Python",description:"Yggdrasil multi-branch reasoning tree decomposition agent with async event loop and structured thought chains.",code:`import asyncio
...
class YggdrasilTreeEngine:
    ...
    async def decompose_query(self, user_prompt: str) -> List[ThoughtBranch]:
        ...
        return branches`},schema:{filename:"schema.sql",language:"SQL",description:"Relational database schema for the Resilient issue discovery queue and agent test matrices.",code:`-- Resilient Autonomous Benchmark Database Schema
...
);`}};function g({onClose:n}){const[s,a]=l.useState("ml"),[r,i]=l.useState(!1),o=c[s]||c.ml,d=()=>{t.playClick(),navigator.clipboard&&(navigator.clipboard.writeText(o.code),i(!0),setTimeout(()=>i(!1),2e3))};return e.jsx("div",{className:"modal-backdrop",onClick:n,role:"presentation",children:e.jsxs("div",{className:"modal-card glass-modal code-modal",onClick:p=>p.stopPropagation(),role:"dialog","aria-modal":"true",children:[e.jsxs("div",{className:"modal-header",children:[e.jsxs("div",{className:"modal-title-group",children:[e.jsx("span",{className:"modal-category",children:"PRODUCTION SOURCE CODE INSPECTOR"}),e.jsx("h2",{className:"modal-title",children:o.filename})]}),e.jsx("button",{type:"button",className:"modal-close-btn",onClick:()=>{t.playClick(),n()},"aria-label":"Close modal",children:"✕"})]}),e.jsxs("div",{className:"code-tabs-bar",children:[e.jsxs("button",{type:"button",className:`code-tab-btn ${s==="ml"?"active":""}`,onClick:()=>{t.playClick(),a("ml")},children:[e.jsx("span",{children:"🏎️"})," autovaluate_stacking.py"]}),e.jsxs("button",{type:"button",className:`code-tab-btn ${s==="phish"?"active":""}`,onClick:()=>{t.playClick(),a("phish")},children:[e.jsx("span",{children:"🛡️"})," custom_transformers.py"]}),e.jsxs("button",{type:"button",className:`code-tab-btn ${s==="pageshield"?"active":""}`,onClick:()=>{t.playClick(),a("pageshield")},children:[e.jsx("span",{children:"🥷"})," background.js"]}),e.jsxs("button",{type:"button",className:`code-tab-btn ${s==="webhook"?"active":""}`,onClick:()=>{t.playClick(),a("webhook")},children:[e.jsx("span",{children:"🤖"})," webhook_receiver.py"]}),e.jsxs("button",{type:"button",className:`code-tab-btn ${s==="yggdrasil"?"active":""}`,onClick:()=>{t.playClick(),a("yggdrasil")},children:[e.jsx("span",{children:"🌲"})," yggdrasil_tree.py"]}),e.jsxs("button",{type:"button",className:`code-tab-btn ${s==="schema"?"active":""}`,onClick:()=>{t.playClick(),a("schema")},children:[e.jsx("span",{children:"🐘"})," schema.sql"]})]}),e.jsxs("div",{className:"code-desc-strip",children:[e.jsx("span",{children:o.description}),e.jsx("button",{type:"button",className:`btn-copy-code ${r?"copied":""}`,onClick:d,title:"Copy code snippet to clipboard",children:e.jsx("span",{children:r?"COPIED TO CLIPBOARD! ✅":"COPY CODE 📋"})})]}),e.jsx("div",{className:"modal-body code-body custom-scroll",children:e.jsx("pre",{className:"code-syntax-display",children:e.jsx("code",{children:o.code})})}),e.jsx("div",{className:"modal-footer",children:e.jsxs("a",{href:"https://github.com/harshitthek",target:"_blank",rel:"noopener noreferrer",className:"btn-modal-launch",onClick:()=>t.playClick(),children:[e.jsx("span",{children:"EXPLORE HARSHIT'S GITHUB REPOSITORIES"})," ↗"]})})]})})}export{g as default};
